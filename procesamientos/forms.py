...
class FormularioCargaNufri(forms.Form):
    semana = forms.IntegerField(
        label="Semana",
        min_value=1,
        max_value=53,
    ...
    pagina_pdf = forms.IntegerField(
        label="Página del PDF",
        min_value=1,
        initial=1,
        help_text=(
            "Número de hoja del PDF multi-liquidación a procesar."
        ),
        error_messages={
            "required": "Indique la página del PDF.",
            "invalid": "La página debe ser un número entero.",
            "min_value": "La página mínima es 1.",
        },
    )
    archivo_despachos = forms.FileField(
        label="Archivo de despachos",
    ...
    def clean_archivo_liquidacion(self):
        ...
        if not nombre.lower().endswith(".pdf"):
            raise forms.ValidationError(
                "La liquidación NUFRI debe ser .pdf."
            )
        return archivo
...
class FormularioMapeoGastoNufri(forms.Form):
    """Formulario dinámico: un campo por rubro no mapeado."""
