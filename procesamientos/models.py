...
def _ruta_base_procesamiento_glamour(
    instance: "ProcesamientoGlamour",
) -> str:
    return f"procesamientos/glamour/{instance.id}"


def ruta_archivo_despachos_glamour(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_glamour(instance)}"
        "/despachos.xlsx"
    )


def ruta_archivo_liquidacion_glamour(
    instance,
    filename: str,
) -> str:
    return (
...
def ruta_archivo_cliente_glamour(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_glamour(instance)}"
        "/cliente.xlsx"
    )
...
def ruta_archivo_resultado_generacion_glamour(
    instance: "GeneracionGlamour",
    filename: str,
) -> str:
    return (
        f"procesamientos/glamour/"
        f"{instance.procesamiento_id}/resultados/"
        f"{instance.id}/resultado.xlsx"
    )


ETIQUETAS_ESTADO_GLAMOUR = {
    "listo": "Listo",
    "pendiente_mapeo_gastos": "Pendiente mapeo de gastos",
    "invalido": "Con errores",
}


class ProcesamientoGlamour(models.Model):
    id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False,
    ...
    rubros_no_mapeados = models.JSONField(
        default=list,
        blank=True,
    )
    resumen_gastos = models.JSONField(
    ...
    archivo_despachos = models.FileField(
        upload_to=ruta_archivo_despachos_glamour,
    )
    archivo_liquidacion = models.FileField(
        upload_to=ruta_archivo_liquidacion_glamour,
    )
    archivo_cliente = models.FileField(
        upload_to=ruta_archivo_cliente_glamour,
    )
    creado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        null=True,
    ...
    @property
    def estado_legible(self) -> str:
        return ETIQUETAS_ESTADO_GLAMOUR.get(
            self.estado,
            self.estado,
        )

    @property
    def tiene_rubros_pendientes(self) -> bool:
        return bool(self.rubros_no_mapeados)


class MapeoGastoGlamour(models.Model):
    etiqueta_normalizada = models.CharField(
        max_length=255,
        unique=True,
    )
    etiqueta_original = models.CharField(max_length=255)
    columna_destino = models.CharField(max_length=100)
    creado_por_nombre = models.CharField(
        max_length=150,
        blank=True,
    )
    ...
    def __str__(self) -> str:
        return (
            f"{self.etiqueta_original} → {self.columna_destino}"
        )

    @classmethod
    def obtener_mapeos_dict(cls) -> dict[str, str]:
        return {
            mapeo.etiqueta_normalizada: mapeo.columna_destino
            for mapeo in cls.objects.all()
        }


class GeneracionGlamour(models.Model):
    class Estado(models.TextChoices):
        PENDIENTE = "pendiente", "Pendiente"
        PROCESANDO = "procesando", "Procesando"
        COMPLETADO = "completado", "Completado"
    ...
    finalizado_en = models.DateTimeField(null=True, blank=True)
    archivo_resultado = models.FileField(
        upload_to=ruta_archivo_resultado_generacion_glamour,
        blank=True,
    )
    nombre_descarga = models.CharField(
        max_length=255,
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name=(
                    "uniq_generacion_glamour_activa"
                    "_por_procesamiento"
                ),
            ),
        ]
...
def _ruta_base_procesamiento_fruver(
    instance: "ProcesamientoFruver",
) -> str:
    return f"procesamientos/fruver/{instance.id}"


def ruta_archivo_despachos_fruver(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_fruver(instance)}/despachos.xlsx"
    )


def ruta_archivo_cliente_fruver(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_fruver(instance)}/cliente.xlsx"
    )


def ruta_archivo_pdf_fruver(
    instance: "ArchivoPdfFruver",
    filename: str,
) -> str:
    nombre = Path(filename or "").name or (
        f"liquidacion_{instance.orden}.pdf"
    ...
    return (
        f"procesamientos/fruver/"
        f"{instance.procesamiento_id}/pdfs/"
        f"{instance.orden:02d}_{nombre}"
    )


def ruta_archivo_resultado_generacion_fruver(
    instance: "GeneracionFruver",
    filename: str,
) -> str:
    return (
        f"procesamientos/fruver/"
...
ETIQUETAS_ESTADO_FRUVER = {
    "listo": "Listo",
    "invalido": "Con errores",
}


class ProcesamientoFruver(models.Model):
    id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False,
    ...
    archivo_despachos = models.FileField(
        upload_to=ruta_archivo_despachos_fruver,
    )
    archivo_cliente = models.FileField(
        upload_to=ruta_archivo_cliente_fruver,
    )
    creado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="procesamientos_fruver_creados",
    )
    ...
    def __str__(self) -> str:
        return (
            f"FRU&VER {self.factura_corta or self.destino_ui} "
            f"W{self.semana} ({self.anio})"
        )
    ...
    @property
    def estado_legible(self) -> str:
        return ETIQUETAS_ESTADO_FRUVER.get(self.estado, self.estado)


class ArchivoPdfFruver(models.Model):
    procesamiento = models.ForeignKey(
        ProcesamientoFruver,
        related_name="pdfs",
        on_delete=models.CASCADE,
    )
    archivo = models.FileField(upload_to=ruta_archivo_pdf_fruver)
...
class GeneracionFruver(models.Model):
    class Estado(models.TextChoices):
        PENDIENTE = "pendiente", "Pendiente"
        PROCESANDO = "procesando", "Procesando"
        COMPLETADO = "completado", "Completado"
    ...
    solicitado_por = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="generaciones_fruver_solicitadas",
    )
    solicitado_por_nombre = models.CharField(max_length=150)
    solicitado_en = models.DateTimeField(auto_now_add=True)
    iniciado_en = models.DateTimeField(null=True, blank=True)
    finalizado_en = models.DateTimeField(null=True, blank=True)
    archivo_resultado = models.FileField(
        upload_to=ruta_archivo_resultado_generacion_fruver,
        blank=True,
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name="uniq_generacion_fruver_activa_por_procesamiento",
            ),
        ]

    def __str__(self) -> str:
        return f"Generación {self.procesamiento_id} ({self.estado})"
...
def _ruta_base_procesamiento_nufri(
    instance: "ProcesamientoNufri",
) -> str:
    return f"procesamientos/nufri/{instance.id}"


def ruta_archivo_despachos_nufri(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_nufri(instance)}"
        "/despachos.xlsx"
    )


def ruta_archivo_liquidacion_nufri(
    instance,
    filename: str,
) -> str:
    return (
...
def ruta_archivo_cliente_nufri(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_nufri(instance)}"
        "/cliente.xlsx"
    )
...
def ruta_archivo_resultado_generacion_nufri(
    instance: "GeneracionNufri",
    filename: str,
) -> str:
    return (
        f"procesamientos/nufri/"
        f"{instance.procesamiento_id}/resultados/"
        f"{instance.id}/resultado.xlsx"
    )


ETIQUETAS_ESTADO_NUFRI = {
    "listo": "Listo",
    "pendiente_mapeo_gastos": "Pendiente mapeo de gastos",
    "invalido": "Con errores",
}
...
class ProcesamientoNufri(models.Model):
    id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False,
    ...
    factura_corta = models.CharField(max_length=20, blank=True)
    destino_ui = models.CharField(max_length=150, blank=True)
    pagina_pdf = models.PositiveIntegerField(default=1)
    estado = models.CharField(max_length=40)
    destino_final = models.CharField(max_length=150, blank=True)
    origen_destino = models.CharField(max_length=30, blank=True)
    destinos_despachos = models.JSONField(
    ...
    archivo_despachos = models.FileField(
        upload_to=ruta_archivo_despachos_nufri,
    )
    archivo_liquidacion = models.FileField(
        upload_to=ruta_archivo_liquidacion_nufri,
    )
    archivo_cliente = models.FileField(
        upload_to=ruta_archivo_cliente_nufri,
    )
    creado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="procesamientos_nufri_creados",
    )
    creado_por_nombre = models.CharField(
        max_length=150,
        blank=True,
    ...
    def __str__(self) -> str:
        return (
            f"NUFRI {self.factura_corta} "
            f"W{self.semana} ({self.anio}) p{self.pagina_pdf}"
        )
    ...
    @property
    def estado_legible(self) -> str:
        return ETIQUETAS_ESTADO_NUFRI.get(
            self.estado,
            self.estado,
        )
...
class MapeoGastoNufri(models.Model):
    etiqueta_normalizada = models.CharField(
        max_length=255,
        unique=True,
    )
...
class GeneracionNufri(models.Model):
    class Estado(models.TextChoices):
        PENDIENTE = "pendiente", "Pendiente"
        PROCESANDO = "procesando", "Procesando"
        COMPLETADO = "completado", "Completado"
    ...
    solicitado_por = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="generaciones_nufri_solicitadas",
    )
    solicitado_por_nombre = models.CharField(max_length=150)
    solicitado_en = models.DateTimeField(auto_now_add=True)
    iniciado_en = models.DateTimeField(null=True, blank=True)
    finalizado_en = models.DateTimeField(null=True, blank=True)
    archivo_resultado = models.FileField(
        upload_to=ruta_archivo_resultado_generacion_nufri,
        blank=True,
    )
    nombre_descarga = models.CharField(
        max_length=255,
...
def _ruta_base_procesamiento_eurobanan(
    instance: "ProcesamientoEurobanan",
) -> str:
    return f"procesamientos/eurobanan/{instance.id}"


def ruta_archivo_despachos_eurobanan(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_eurobanan(instance)}"
        "/despachos.xlsx"
    )


def ruta_archivo_liquidacion_eurobanan(
    instance,
    filename: str,
) -> str:
    return (
...
def ruta_archivo_cliente_eurobanan(instance, filename: str) -> str:
    return (
        f"{_ruta_base_procesamiento_eurobanan(instance)}"
        "/cliente.xlsx"
    )
...
def ruta_archivo_resultado_generacion_eurobanan(
    instance: "GeneracionEurobanan",
    filename: str,
) -> str:
    return (
        f"procesamientos/eurobanan/"
        f"{instance.procesamiento_id}/resultados/"
        f"{instance.id}/resultado.xlsx"
    )


ETIQUETAS_ESTADO_EUROBANAN = {
    "listo": "Listo",
    "pendiente_mapeo_gastos": "Pendiente mapeo de gastos",
    "invalido": "Con errores",
}
...
class ProcesamientoEurobanan(models.Model):
    id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False,
    ...
    archivo_despachos = models.FileField(
        upload_to=ruta_archivo_despachos_eurobanan,
    )
    archivo_liquidacion = models.FileField(
        upload_to=ruta_archivo_liquidacion_eurobanan,
    )
    archivo_cliente = models.FileField(
        upload_to=ruta_archivo_cliente_eurobanan,
    )
    creado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="procesamientos_eurobanan_creados",
    )
    creado_por_nombre = models.CharField(
        max_length=150,
        blank=True,
    ...
    def __str__(self) -> str:
        return (
            f"EUROBANAN {self.factura_corta} "
            f"W{self.semana} ({self.anio})"
        )
    ...
    @property
    def estado_legible(self) -> str:
        return ETIQUETAS_ESTADO_EUROBANAN.get(
            self.estado,
            self.estado,
        )
...
class MapeoGastoEurobanan(models.Model):
    etiqueta_normalizada = models.CharField(
        max_length=255,
        unique=True,
    )
...
class GeneracionEurobanan(models.Model):
    class Estado(models.TextChoices):
        PENDIENTE = "pendiente", "Pendiente"
        PROCESANDO = "procesando", "Procesando"
        COMPLETADO = "completado", "Completado"
    ...
    solicitado_por = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="generaciones_eurobanan_solicitadas",
    )
    solicitado_por_nombre = models.CharField(max_length=150)
    solicitado_en = models.DateTimeField(auto_now_add=True)
    iniciado_en = models.DateTimeField(null=True, blank=True)
    finalizado_en = models.DateTimeField(null=True, blank=True)
    archivo_resultado = models.FileField(
        upload_to=ruta_archivo_resultado_generacion_eurobanan,
        blank=True,
    )
    nombre_descarga = models.CharField(
        max_length=255,
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name=(
                    "uniq_generacion_eurobanan_activa"
                    "_por_procesamiento"
                ),
            ),
        ]
