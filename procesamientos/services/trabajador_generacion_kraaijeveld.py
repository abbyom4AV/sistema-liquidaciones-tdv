...
from procesamientos.models import GeneracionKraaijeveld
from procesamientos.services.generacion_kraaijeveld import (
    ErrorConfirmacionGeneracionKraaijeveld,
...
from services.kraaijeveld.writer import (
    ErrorEscrituraKraaijeveld,
    escribir_archivo_kraaijeveld,
...
class WorkerLockKraaijeveldError(RuntimeError):
    """No se pudo adquirir el bloqueo del trabajador Kraaijeveld."""


class WorkerLockKraaijeveld:
    def __init__(self, ruta: Path | None = None):
        self.ruta = ruta or (
            Path(settings.BASE_DIR)
            / "runtime"
            / "kraaijeveld_worker.lock"
        )
        self._fd: int | None = None

    def __enter__(self) -> "WorkerLockKraaijeveld":
        ...
        try:
            ...
            raise WorkerLockKraaijeveldError(
                "Ya hay un trabajador Kraaijeveld en ejecución. Si "
...
def reclamar_siguiente_generacion_kraaijeveld() -> (
    GeneracionKraaijeveld | None
):
    with transaction.atomic():
        generacion = (
            GeneracionKraaijeveld.objects.select_for_update()
            .filter(estado=GeneracionKraaijeveld.Estado.PENDIENTE)
            .order_by("solicitado_en")
            .first()
        )
        if generacion is None:
            return None
        generacion.estado = GeneracionKraaijeveld.Estado.PROCESANDO
...
def _marcar_error(
    generacion: GeneracionKraaijeveld,
    mensaje: str,
) -> None:
    generacion.estado = GeneracionKraaijeveld.Estado.ERROR
...
def procesar_generacion_kraaijeveld(
    generacion: GeneracionKraaijeveld,
) -> None:
    ...
    try:
        ...
        except ErrorConfirmacionGeneracionKraaijeveld as error:
            _marcar_error(generacion, str(error))
            return
        logger.info(
            "generacion_kraaijeveld=%s fase=reconstruir_desde_bd "
            "segundos=%.3f",
            generacion.id,
        ...
        escritura = escribir_archivo_kraaijeveld(
            ...
            recalcular_al_final=bool(
                getattr(
                    settings,
                    "KRAAIJEVELD_RECALCULAR_AL_FINAL",
                    False,
                )
            ),
        )
        ...
        generacion.estado = GeneracionKraaijeveld.Estado.COMPLETADO
        generacion.mensaje_error = ""
        generacion.finalizado_en = timezone.now()
        generacion.save()
        ...
        logger.exception(
            "Error en generación Kraaijeveld %s",
            generacion.id,
        )
        if ruta_salida.exists():
        ...
        logger.exception(
            "Error inesperado en generación Kraaijeveld %s",
            generacion.id,
        )
        if ruta_salida.exists():
            ruta_salida.unlink(missing_ok=True)
