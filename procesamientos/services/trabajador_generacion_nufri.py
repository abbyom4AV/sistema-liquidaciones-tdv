...
from procesamientos.models import GeneracionNufri
from procesamientos.services.generacion_nufri import (
    ErrorConfirmacionGeneracionNufri,
    construir_nombre_descarga,
    reconstruir_resultado_para_escritura_nufri,
)
from services.nufri.extractor import ErrorExtraccionNufri
from services.nufri.matcher import ErrorMatcherNufri
from services.nufri.processor import ErrorProcesamientoNufri
from services.nufri.writer import (
    ErrorEscrituraNufri,
    escribir_archivo_nufri,
...
class WorkerLockNufriError(RuntimeError):
    """No se pudo adquirir el bloqueo del trabajador Nufri."""


class WorkerLockNufri:
    ...
    def __enter__(self) -> "WorkerLockNufri":
        ...
        try:
            ...
            raise WorkerLockNufriError(
                "Ya hay un trabajador Nufri en ejecución. Si "
...
def reclamar_siguiente_generacion_nufri() -> (
    GeneracionNufri | None
):
    with transaction.atomic():
        generacion = (
            GeneracionNufri.objects.select_for_update()
            .filter(estado=GeneracionNufri.Estado.PENDIENTE)
            .order_by("solicitado_en")
            .first()
        )
        if generacion is None:
            return None
        generacion.estado = GeneracionNufri.Estado.PROCESANDO
...
def _marcar_error(
    generacion: GeneracionNufri,
    mensaje: str,
) -> None:
    generacion.estado = GeneracionNufri.Estado.ERROR
...
def procesar_generacion_nufri(
    ...
    try:
        ...
        try:
            resultado = reconstruir_resultado_para_escritura_nufri(
        ...
        except ErrorConfirmacionGeneracionNufri as error:
            _marcar_error(generacion, str(error))
            return
        logger.info(
            "generacion_nufri=%s fase=reconstruir_desde_bd "
            "segundos=%.3f",
            generacion.id,
        ...
        escritura = escribir_archivo_nufri(
        ...
        generacion.estado = GeneracionNufri.Estado.COMPLETADO
        generacion.mensaje_error = ""
        generacion.finalizado_en = timezone.now()
        generacion.save()
        ...
        logger.exception(
            "Error inesperado en generación Nufri %s",
            generacion.id,
        )
        if ruta_salida.exists():
            ruta_salida.unlink(missing_ok=True)
