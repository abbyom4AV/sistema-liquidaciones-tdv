...
class ErrorConfirmacionGeneracionEurobanan(Exception):
    """Error al reconstruir el procesamiento Eurobanan para escritura."""


def construir_nombre_descarga() -> str:
    return NOMBRE_DESCARGA_EUROBANAN
...
def _a_decimal(valor: Any, campo: str) -> Decimal:
    try:
        ...
        raise ErrorConfirmacionGeneracionEurobanan(
...
def serializar_linea_preparada_eurobanan(
    linea: LineaPreparadaEurobanan,
) -> dict[str, Any]:
...
def serializar_lineas_preparadas_eurobanan(lineas) -> list[dict[str, Any]]:
    return [
        serializar_linea_preparada_eurobanan(linea)
        for linea in lineas
    ]


def serializar_resumen_gastos_eurobanan(
...
def reconstruir_resultado_para_escritura_eurobanan(
    *,
    factura_corta: str,
    semana: int,
    anio: int,
    ...
    lineas: list[LineaPreparadaEurobanan] = []
