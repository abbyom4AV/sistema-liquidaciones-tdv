...
from procesamientos.models import GeneracionTdvEuropa
from procesamientos.services.generacion_tdv_europa import (
    ErrorConfirmacionGeneracionTdvEuropa,
    construir_nombre_descarga,
    reconstruir_resultado_para_escritura_tdv_europa,
)
...
from services.tdv_europa.processor import ErrorProcesamientoTdvEuropa
from services.tdv_europa.writer import (
    ErrorEscrituraTdvEuropa,
    escribir_archivo_tdv_europa,
...
class WorkerLockTdvEuropaError(RuntimeError):
    """No se pudo adquirir el bloqueo del trabajador TDV Europa."""


class WorkerLockTdvEuropa:
    def __init__(self, ruta: Path | None = None):
        self.ruta = ruta or (
            Path(settings.BASE_DIR)
            / "runtime"
            / "tdv_europa_worker.lock"
        )
        self._fd: int | None = None

    def __enter__(self) -> "WorkerLockTdvEuropa":
        ...
        try:
            ...
            raise WorkerLockTdvEuropaError(
                "Ya hay un trabajador TDV Europa en ejecución. Si "
...
def reclamar_siguiente_generacion_tdv_europa() -> (
    GeneracionTdvEuropa | None
):
    with transaction.atomic():
        generacion = (
            GeneracionTdvEuropa.objects.select_for_update()
            .filter(estado=GeneracionTdvEuropa.Estado.PENDIENTE)
            .order_by("solicitado_en")
            .first()
        )
        if generacion is None:
            return None
        generacion.estado = GeneracionTdvEuropa.Estado.PROCESANDO
...
def _marcar_error(
    generacion: GeneracionTdvEuropa,
    mensaje: str,
) -> None:
    generacion.estado = GeneracionTdvEuropa.Estado.ERROR
...
def procesar_generacion_tdv_europa(
    generacion: GeneracionTdvEuropa,
) -> None:
    ...
    try:
        ...
        try:
            resultado = reconstruir_resultado_para_escritura_tdv_europa(
                ...
                total_merma=procesamiento.total_merma,
                reclamos_irmadona=procesamiento.reclamos_irmadona,
                reclamos_mercado=procesamiento.reclamos_mercado,
                comision_eur=procesamiento.comision_eur,
                destinos_despachos=procesamiento.destinos_despachos,
            )
        except ErrorConfirmacionGeneracionTdvEuropa as error:
            _marcar_error(generacion, str(error))
            return
        logger.info(
            "generacion_tdv_europa=%s fase=reconstruir_desde_bd "
            "segundos=%.3f",
            generacion.id,
        ...
        escritura = escribir_archivo_tdv_europa(
        ...
        generacion.estado = GeneracionTdvEuropa.Estado.COMPLETADO
        generacion.mensaje_error = ""
        generacion.finalizado_en = timezone.now()
        generacion.save()
        ...
        logger.exception(
            "Error en generación TDV Europa %s",
            generacion.id,
        )
        if ruta_salida.exists():
        ...
        logger.exception(
            "Error inesperado en generación TDV Europa %s",
            generacion.id,
        )
        if ruta_salida.exists():
            ruta_salida.unlink(missing_ok=True)
