...
from procesamientos.models import GeneracionGlamour
from procesamientos.services.generacion_glamour import (
    ErrorConfirmacionGeneracionGlamour,
    construir_nombre_descarga,
    reconstruir_resultado_para_escritura_glamour,
)
from services.glamour.extractor import ErrorExtraccionGlamour
from services.glamour.matcher import ErrorMatcherGlamour
from services.glamour.processor import ErrorProcesamientoGlamour
from services.glamour.writer import (
    ErrorEscrituraGlamour,
...
class WorkerLockGlamourError(RuntimeError):
    """No se pudo adquirir el bloqueo del trabajador Glamour."""


class WorkerLockGlamour:
    ...
    def __enter__(self) -> "WorkerLockGlamour":
        ...
        try:
            ...
            raise WorkerLockGlamourError(
                "Ya hay un trabajador Glamour en ejecución. Si "
...
def reclamar_siguiente_generacion_glamour() -> (
    GeneracionGlamour | None
):
    with transaction.atomic():
        generacion = (
            GeneracionGlamour.objects.select_for_update()
            .filter(estado=GeneracionGlamour.Estado.PENDIENTE)
            .order_by("solicitado_en")
            .first()
        )
        if generacion is None:
            return None
        generacion.estado = GeneracionGlamour.Estado.PROCESANDO
...
def _marcar_error(
    generacion: GeneracionGlamour,
    mensaje: str,
) -> None:
    generacion.estado = GeneracionGlamour.Estado.ERROR
...
def procesar_generacion_glamour(
    generacion: GeneracionGlamour,
) -> None:
    ...
    try:
        ...
        except ErrorConfirmacionGeneracionGlamour as error:
            _marcar_error(generacion, str(error))
            return
        logger.info(
            "generacion_glamour=%s fase=reconstruir_desde_bd "
            "segundos=%.3f",
            generacion.id,
        ...
        generacion.estado = GeneracionGlamour.Estado.COMPLETADO
        generacion.mensaje_error = ""
        generacion.finalizado_en = timezone.now()
        generacion.save()
        ...
        logger.exception(
            "Error inesperado en generación Glamour %s",
            generacion.id,
        )
        if ruta_salida.exists():
            ruta_salida.unlink(missing_ok=True)
