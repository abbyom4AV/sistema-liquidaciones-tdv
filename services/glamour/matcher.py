...
CLIENTE_GLAMOUR = "GLAMOUR FRESH"


class ErrorMatcherGlamour(Exception):
    """Error general al cruzar Despachos para Glamour."""


class FormatoDespachosGlamourError(ErrorMatcherGlamour):
    """El archivo de Despachos no tiene el formato esperado."""


class SinCoincidenciasGlamourError(ErrorMatcherGlamour):
    """No hay líneas Glamour Fresh para los criterios."""


@dataclass(frozen=True)
class LineaDespachoGlamour:
    fila_excel: int
    semana: int
    anio: int
    semana_texto: str
...
class ResultadoMatcherGlamour:
    ...
    destino_buscado: str
    semana_texto: str
    lineas: tuple[LineaDespachoGlamour, ...]
    total_cajas: int
...
def buscar_lineas_despachos_glamour(
    ...
    *,
    ...
    """
    Filtra Despachos por GLAMOUR FRESH + semana + año + destino
    + factura (4 dígitos). Los filtros de UI evitan recorrer
    todo el histórico solo por factura.
    """
    ruta = Path(ruta_archivo)
    if not ruta.is_file():
        raise ErrorMatcherGlamour(
            f"No existe el archivo de Despachos: {ruta}"
        )
    if not (1 <= int(semana) <= 53):
        raise FormatoDespachosGlamourError(
    ...
    try:
        ...
        try:
            ...
            raise FormatoDespachosGlamourError(str(error)) from error

        lineas: list[LineaDespachoGlamour] = []
        ...
        if not lineas:
            raise SinCoincidenciasGlamourError(
        ...
        return ResultadoMatcherGlamour(
            archivo=ruta.name,
            hoja="Base Datos",
        ...
        raise SinCoincidenciasGlamourError(str(error)) from error
    finally:
        libro.close()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Busca líneas Despachos Glamour Fresh.",
    )
    ...
    parser.add_argument("--cliente", default=CLIENTE_GLAMOUR)
