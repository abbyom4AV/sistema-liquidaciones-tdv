...
NOMBRE_DESCARGA_GLAMOUR = "Glamour Liquidaciones V1.xlsx"
...
def _resolver_rutas_raw_data(
    ...
    try:
        ...
        raise EstructuraRawDataGlamourError(
...
class ErrorEscrituraGlamour(Exception):
    """Error general al escribir el acumulativo Glamour."""


class EstructuraRawDataGlamourError(ErrorEscrituraGlamour):
    """Tabla1 no tiene la estructura esperada."""


class ContenedorDuplicadoGlamourError(ErrorEscrituraGlamour):
    """Uno de los contenedores ya está en Raw Data."""


class ProcesamientoGlamourNoListoError(ErrorEscrituraGlamour):
    """El procesamiento no está listo para escribir."""


@dataclass(frozen=True)
class ResultadoEscrituraGlamour:
    archivo_origen: str
    archivo_salida: str
    filas_agregadas: int
    fila_inicial: int
...
def _log_fase(fase: str, segundos: float) -> None:
    logger.info(
        "generacion_glamour=%s fase=%s segundos=%.3f",
...
def construir_valores_fila_glamour(
    procesamiento: ResultadoPreparacionGlamour,
    indice_linea: int,
) -> dict[str, Any]:
...
def escribir_archivo_glamour(
    ...
    if not procesamiento.puede_escribir:
        raise ProcesamientoGlamourNoListoError(
            "El procesamiento no está listo para escribir. "
    ...
    carpeta_trabajo = Path(
        tempfile.mkdtemp(prefix="glamour_write_")
    )
    salida_trabajo = carpeta_trabajo / "trabajo.xlsx"
    escritura_completada = False
    resultado: ResultadoEscrituraGlamour | None = None
    ...
    try:
        ...
        with ZipFile(salida_trabajo, "r") as zipped:
            ...
            if duplicado:
                raise ContenedorDuplicadoGlamourError(
        ...
        for offset in range(cantidad):
            ...
            valores = construir_valores_fila_glamour(
            ...
            if offset == 0:
                logger.info(
                    "generacion_glamour=%s hoja=%s tabla=%s "
        ...
        resultado = ResultadoEscrituraGlamour(
