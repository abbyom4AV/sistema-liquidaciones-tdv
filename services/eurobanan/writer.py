...
from services.eurobanan.processor import ResultadoPreparacionEurobanan

logger = logging.getLogger(__name__)

_contexto_generacion: contextvars.ContextVar[str] = (
    contextvars.ContextVar(
        "eurobanan_generacion_id",
        default="-",
    )
)
...
NOMBRE_DESCARGA_EUROBANAN = "EUROBANAN Liquidaciones.xlsx"
...
def _resolver_rutas_raw_data(
    ...
    try:
        ...
        raise EstructuraRawDataEurobananError(
...
class ErrorEscrituraEurobanan(Exception):
    """Error general al escribir el acumulativo Eurobanan."""


class EstructuraRawDataEurobananError(ErrorEscrituraEurobanan):
    """Tabla1 no tiene la estructura esperada."""


class ContenedorDuplicadoEurobananError(ErrorEscrituraEurobanan):
    """Uno de los contenedores ya está en Raw Data."""


class ProcesamientoEurobananNoListoError(ErrorEscrituraEurobanan):
    """El procesamiento no está listo para escribir."""


@dataclass(frozen=True)
class ResultadoEscrituraEurobanan:
    archivo_origen: str
    archivo_salida: str
    filas_agregadas: int
    fila_inicial: int
...
def _log_fase(fase: str, segundos: float) -> None:
    logger.info(
        "generacion_eurobanan=%s fase=%s segundos=%.3f",
...
def construir_valores_fila_eurobanan(
    procesamiento: ResultadoPreparacionEurobanan,
    indice_linea: int,
) -> dict[str, Any]:
    ...
    fila["Comision €"] = decimal_a_excel(linea.comision_eur)
    return fila
...
def escribir_archivo_eurobanan(
    ...
    if not procesamiento.puede_escribir:
        raise ProcesamientoEurobananNoListoError(
            "El procesamiento no está listo para escribir. "
    ...
    carpeta_trabajo = Path(
        tempfile.mkdtemp(prefix="eurobanan_write_")
    )
    salida_trabajo = carpeta_trabajo / "trabajo.xlsx"
    escritura_completada = False
    resultado: ResultadoEscrituraEurobanan | None = None
    ...
    try:
        ...
        with ZipFile(salida_trabajo, "r") as zipped:
            ...
            if duplicado:
                raise ContenedorDuplicadoEurobananError(
        ...
        for offset in range(cantidad):
            ...
            valores = construir_valores_fila_eurobanan(
            ...
            if offset == 0:
                logger.info(
                    "generacion_eurobanan=%s hoja=%s tabla=%s "
        ...
        resultado = ResultadoEscrituraEurobanan(
