...
CLIENTE_KRAAIJEVELD = "KRAAIJEVELD"


class ErrorMatcherKraaijeveld(Exception):
    """Error general al cruzar Despachos para Kraaijeveld."""


class FormatoDespachosKraaijeveldError(ErrorMatcherKraaijeveld):
    """El archivo de Despachos no tiene el formato esperado."""


class SinCoincidenciasKraaijeveldError(ErrorMatcherKraaijeveld):
    """No hay líneas Kraaijeveld para los criterios."""


@dataclass(frozen=True)
class LineaDespachoKraaijeveld:
    fila_excel: int
    semana: int
    anio: int
    semana_texto: str
    ...
    def tipo_fruta(self) -> str:
        return texto_limpio(self.tipo_empaque) or self.tipo_empaque


@dataclass(frozen=True)
class ResultadoMatcherKraaijeveld:
    archivo: str
    hoja: str
    cliente_buscado: str
    semana: int
    ...
    destino_buscado: str
    semana_texto: str
    lineas: tuple[LineaDespachoKraaijeveld, ...]
    total_cajas: int
...
def buscar_lineas_despachos_kraaijeveld(
    ...
    try:
        ...
        lineas: list[LineaDespachoKraaijeveld] = []
        for numero_fila, fila in enumerate(
            ...
            try:
                ...
                linea = LineaDespachoKraaijeveld(
        ...
        if not lineas:
            raise SinCoincidenciasKraaijeveldError(
                "No se encontraron líneas en Despachos para "
                f"KRAAIJEVELD, semana {semana}, año {anio}, "
                f"destino '{destino}'."
            )
        ...
        return ResultadoMatcherKraaijeveld(
            archivo=ruta.name,
            hoja="Base Datos",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Busca líneas Despachos Kraaijeveld.",
    )
