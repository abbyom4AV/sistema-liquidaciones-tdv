...
from services.sifa.extractor import COLUMNAS_GASTO
...
NOMBRE_DESCARGA_SIFA = "SIFA Liquidaciones V3.xlsx"
...
ALIAS_COLUMNAS = {
    ...
    "Deduction Eur": ("Deduction Eur", "Deduction Eur "),
    "Fresco Eur": ("Fresco Eur", "Fresco Eur "),
    "Transportation Eur": (
        "Transportation Eur",
        "Transportation Eur ",
    ),
    "Logistica Eur": ("Logistica Eur", "Logistica Eur "),
    "Handling Eur": ("Handling Eur", "Handling Eur "),
    "Transportation Cus. Eur": (
        "Transportation Cus. Eur",
        "Transportation Cus. Eur ",
    ),
...
def _resolver_rutas_raw_data(
    ...
    try:
        ...
        raise EstructuraRawDataSifaError(
...
class ErrorEscrituraSifa(Exception):
    """Error general al escribir el acumulativo sifa."""


class EstructuraRawDataSifaError(ErrorEscrituraSifa):
    """Tabla1 no tiene la estructura esperada."""


class ContenedorDuplicadoSifaError(ErrorEscrituraSifa):
    """Uno de los contenedores ya está en Raw Data."""


class ProcesamientoSifaNoListoError(ErrorEscrituraSifa):
    """El procesamiento no está listo para escribir."""


@dataclass(frozen=True)
class ResultadoEscrituraSifa:
    archivo_origen: str
    archivo_salida: str
    filas_agregadas: int
    fila_inicial: int
...
def _log_fase(fase: str, segundos: float) -> None:
    logger.info(
        "generacion_sifa=%s fase=%s segundos=%.3f",
...
def construir_valores_fila_sifa(
    procesamiento: ResultadoPreparacionSifa,
    indice_linea: int,
) -> dict[str, Any]:
...
def escribir_archivo_sifa(
    ...
    if not procesamiento.puede_escribir:
        raise ProcesamientoSifaNoListoError(
            "El procesamiento no está listo para escribir. "
    ...
    carpeta_trabajo = Path(
        tempfile.mkdtemp(prefix="sifa_write_")
    )
    salida_trabajo = carpeta_trabajo / "trabajo.xlsx"
    escritura_completada = False
    resultado: ResultadoEscrituraSifa | None = None
    ...
    try:
        ...
        with ZipFile(salida_trabajo, "r") as zipped:
            ...
            if duplicado:
                raise ContenedorDuplicadoSifaError(
        ...
        for offset in range(cantidad):
            ...
            valores = construir_valores_fila_sifa(
            ...
            if offset == 0:
                logger.info(
                    "generacion_sifa=%s hoja=%s tabla=%s "
        ...
        resultado = ResultadoEscrituraSifa(
