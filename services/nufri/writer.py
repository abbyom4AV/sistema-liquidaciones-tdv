...
from services.nufri.extractor import COLUMNAS_GASTO
...
NOMBRE_DESCARGA_NUFRI = "NUFRI Liquidaciones Original.xlsx"
...
def _resolver_rutas_raw_data(
    ...
    try:
        ...
        raise EstructuraRawDataNufriError(
...
class ErrorEscrituraNufri(Exception):
    """Error general al escribir el acumulativo Nufri."""


class EstructuraRawDataNufriError(ErrorEscrituraNufri):
    """Tabla1 no tiene la estructura esperada."""


class ContenedorDuplicadoNufriError(ErrorEscrituraNufri):
    """Uno de los contenedores ya está en Raw Data."""


class ProcesamientoNufriNoListoError(ErrorEscrituraNufri):
    """El procesamiento no está listo para escribir."""


@dataclass(frozen=True)
class ResultadoEscrituraNufri:
    archivo_origen: str
    archivo_salida: str
    filas_agregadas: int
    fila_inicial: int
...
def _log_fase(fase: str, segundos: float) -> None:
    logger.info(
        "generacion_nufri=%s fase=%s segundos=%.3f",
...
def _celda_valor_xml(
    ...
    if isinstance(valor, Decimal):
        # Conservar todos los decimales (sin pasar por float).
        return (
            f'<c r="{ref}"{base}>'
            f"<v>{format(valor, 'f')}</v></c>"
        )
...
def construir_valores_fila_nufri(
    procesamiento: ResultadoPreparacionNufri,
    indice_linea: int,
) -> dict[str, Any]:
    ...
    fila: dict[str, Any] = {
        ...
        "Precio de Venta €": linea.precio_venta_eur,
    }
...
def escribir_archivo_nufri(
    ...
    if not procesamiento.puede_escribir:
        raise ProcesamientoNufriNoListoError(
            "El procesamiento no está listo para escribir. "
    ...
    carpeta_trabajo = Path(
        tempfile.mkdtemp(prefix="nufri_write_")
    )
    salida_trabajo = carpeta_trabajo / "trabajo.xlsx"
    escritura_completada = False
    resultado: ResultadoEscrituraNufri | None = None
    ...
    try:
        ...
        with ZipFile(salida_trabajo, "r") as zipped:
            ...
            if duplicado:
                raise ContenedorDuplicadoNufriError(
        ...
        for offset in range(cantidad):
            ...
            valores = construir_valores_fila_nufri(
            ...
            if offset == 0:
                logger.info(
                    "generacion_nufri=%s hoja=%s tabla=%s "
        ...
        resultado = ResultadoEscrituraNufri(
