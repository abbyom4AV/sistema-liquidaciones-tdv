...
from services.visafruits.extractor import COLUMNAS_GASTO
...
NOMBRE_DESCARGA_VISAFRUITS = "VISAFRUITS Liquidaciones.xlsx"
...
# Las 14 columnas que digita el sistema. El resto de Tabla1 son
# fórmulas que el usuario baja a mano al abrir el acumulativo.
COLUMNAS_ENTRADA = {
    "Semana",
    "Año",
    "Cliente",
...
ALIAS_COLUMNAS = {
    ...
    "Total Cajas": ("Total Cajas", "Total Cajas "),
    "Cartón": ("Cartón", "Cartón ", "Carton"),
    "# Calibre": ("# Calibre", "# Calibre "),
    "Tipo de fruta": ("Tipo de fruta", "Tipo de Fruta"),
...
class ErrorEscrituraVisafruits(Exception):
    """Error general al escribir el acumulativo de VISAFRUITS."""


class EstructuraRawDataVisafruitsError(ErrorEscrituraVisafruits):
    """Tabla1 no tiene la estructura esperada."""


class ContenedorDuplicadoVisafruitsError(ErrorEscrituraVisafruits):
    """Uno de los contenedores ya está en Raw Data."""


class ProcesamientoVisafruitsNoListoError(ErrorEscrituraVisafruits):
    """El procesamiento no está listo para escribir."""


@dataclass(frozen=True)
class ResultadoEscrituraVisafruits:
    archivo_origen: str
    archivo_salida: str
    filas_agregadas: int
    fila_inicial: int
...
def _log_fase(fase: str, segundos: float) -> None:
    logger.info(
        "generacion_visafruits=%s fase=%s segundos=%.3f",
...
def _resolver_rutas_raw_data(zipped: ZipFile) -> tuple[str, str]:
    """Ubica el XML de la hoja 'Raw Data' y el de 'Tabla1' sin
    asumir el índice de hoja."""
    try:
        workbook_xml = zipped.read("xl/workbook.xml").decode("utf-8")
        rels_xml = zipped.read(
            "xl/_rels/workbook.xml.rels"
        ).decode("utf-8")
    except KeyError as error:
        raise EstructuraRawDataVisafruitsError(
    ...
    hoja_xml = target if target.startswith("xl/") else f"xl/{target}"
    ...
    for ruta_tabla in candidatos:
        ...
        if (
            ...
            or f"name='{TABLA_RAW_DATA}'" in crudo
        ):
            return hoja_xml, ruta_tabla

    raise EstructuraRawDataVisafruitsError(
        f"No se encontró la tabla {TABLA_RAW_DATA!r} en la hoja "
        f"{HOJA_RAW_DATA!r}."
    )
...
def _cargar_shared_strings(zipped: ZipFile) -> list[str]:
    ...
    for si in root.findall("m:si", ns):
        ...
        cadenas.append("".join(textos) if textos else "")
    return cadenas
...
def _celdas_de_fila_xml(
    ...
    for match in _CELL_RE.finditer(fila_xml):
        ...
        valores[column_index_from_string(letra)] = (
            _valor_desde_celda_xml(attrs, cuerpo, shared)
        )
    return valores
...
def _leer_encabezados_xml(
    ...
    for col in range(col_ini, col_fin + 1):
        ...
        if nombre and nombre in encabezados:
            raise EstructuraRawDataVisafruitsError(
                f"El encabezado {nombre!r} está repetido en Tabla1."
            )
...
def _existe_duplicado_xml(
    ...
    *,
    ...
    buscados = {c.strip().upper() for c in contenedores if c.strip()}
...
def _fila_digitada_xml(
    ...
    *,
    ...
    """Solo columnas digitadas; las fórmulas se bajan a mano."""
...
def construir_valores_fila_visafruits(
    procesamiento: ResultadoPreparacionVisafruits,
    indice_linea: int,
) -> dict[str, Any]:
    linea = procesamiento.validacion.lineas_preparadas[indice_linea]
    semana_texto = linea.semana_texto or (
        f"{linea.semana:02d}-{linea.anio}"
    )
    ...
    fila: dict[str, Any] = {
        ...
        "Precio de Venta €": decimal_a_excel(linea.precio_venta_eur),
    }
...
def _parsear_ref_tabla(ref: str) -> tuple[int, int, int, int]:
    ...
    return (
        column_index_from_string(match_a.group(1)),
        int(match_a.group(2)),
        column_index_from_string(match_b.group(1)),
        int(match_b.group(2)),
    )
...
def _extraer_ref_tabla(tabla_xml: str) -> str:
    ...
    if match is None:
        raise EstructuraRawDataVisafruitsError(
            "No se encontró ref en el XML de Tabla1."
        )
...
def _insertar_filas_en_sheet(sheet_xml: str, filas_xml: str) -> str:
    marca = "</sheetData>"
    idx = sheet_xml.rfind(marca)
    if idx < 0:
        raise ErrorEscrituraVisafruits(
...
def _reescribir_xlsx(
    ...
    *,
    ...
    with ZipFile(ruta, "r") as zin, ZipFile(temporal, "w") as zout:
...
def escribir_archivo_visafruits(
    procesamiento: ResultadoPreparacionVisafruits,
    ruta_archivo_cliente: str | Path,
    ruta_salida: str | Path,
    recalcular_al_final: bool = False,
) -> ResultadoEscrituraVisafruits:
    """Agrega las filas digitadas a Tabla1 parcheando el XML.

    No abre Excel ni copia fórmulas: al abrir el acumulativo el
    usuario baja las fórmulas. Así se preservan tablas dinámicas,
    segmentaciones y la Tabla2 que vive en la misma hoja.
    """
    del recalcular_al_final

    if not procesamiento.puede_escribir:
        raise ProcesamientoVisafruitsNoListoError(
            "El procesamiento no está listo para escribir. "
    ...
    carpeta_trabajo = Path(
        tempfile.mkdtemp(prefix="visafruits_write_")
    )
    salida_trabajo = carpeta_trabajo / "trabajo.xlsx"
    escritura_completada = False
    resultado: ResultadoEscrituraVisafruits | None = None

    try:
        inicio = time.perf_counter()
        shutil.copy2(origen, salida_trabajo)
        _log_fase("copiar_acumulativo", time.perf_counter() - inicio)

        inicio = time.perf_counter()
        with ZipFile(salida_trabajo, "r") as zipped:
            hoja_xml_path, tabla_xml_path = _resolver_rutas_raw_data(
                zipped
            )
            shared = _cargar_shared_strings(zipped)
            tabla_xml = zipped.read(tabla_xml_path).decode("utf-8")
            sheet_xml = zipped.read(hoja_xml_path).decode("utf-8")

            ref_actual = _extraer_ref_tabla(tabla_xml)
            col_ini, fila_enc, col_fin, fila_fin = _parsear_ref_tabla(
                ref_actual
            )
            if fila_enc != 1:
                raise EstructuraRawDataVisafruitsError(
                    "Se esperaba el encabezado de Tabla1 en la fila 1."
                )
            ...
            destino = procesamiento.validacion.destino_aplicado
            contenedores = {
                linea.contenedor
                for linea in (
            ...
            if duplicado:
                raise ContenedorDuplicadoVisafruitsError(
                    f"El contenedor {duplicado} ya existe en Raw Data "
                    f"para la semana {procesamiento.despachos.semana}, "
        ...
        cantidad = len(procesamiento.validacion.lineas_preparadas)
        ...
        for offset in range(cantidad):
            valores = construir_valores_fila_visafruits(
            ...
            if offset == 0:
                logger.info(
                    "generacion_visafruits=%s hoja=%s tabla=%s "
                    "digitado_cols=%s (sin formulas)",
                    _contexto_generacion.get(),
                    hoja_xml_path,
                    tabla_xml_path,
                    len(valores_por_col),
        ...
        _log_fase("guardar_zip", time.perf_counter() - inicio)

        shutil.copy2(salida_trabajo, salida)
        escritura_completada = True
        resultado = ResultadoEscrituraVisafruits(
        ...
        _log_fase("total_writer", time.perf_counter() - inicio_total)
