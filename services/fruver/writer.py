...
NOMBRE_DESCARGA_FRUVER = "FRU&VER Liquidaciones.xlsx"
...
def _resolver_rutas_raw_data(
    ...
    try:
        ...
        raise EstructuraRawDataFruverError(
...
class ErrorEscrituraFruver(Exception):
    """Error general al escribir el acumulativo TDV Europa."""


class EstructuraRawDataFruverError(ErrorEscrituraFruver):
    """Tabla1 no tiene la estructura esperada."""


class ContenedorDuplicadoFruverError(ErrorEscrituraFruver):
    """Uno de los contenedores ya está en Raw Data."""


class ProcesamientoFruverNoListoError(ErrorEscrituraFruver):
    """El procesamiento no está listo para escribir."""


@dataclass(frozen=True)
class ResultadoEscrituraFruver:
    archivo_origen: str
    archivo_salida: str
    filas_agregadas: int
    fila_inicial: int
...
def _log_fase(fase: str, segundos: float) -> None:
    logger.info(
        "generacion_fruver=%s fase=%s segundos=%.3f",
...
def construir_valores_fila_fruver(
    procesamiento: ResultadoPreparacionFruver,
    indice_linea: int,
) -> dict[str, Any]:
    ...
    return {
        ...
        "Cartón": linea.carton,
        "Demora Eur": decimal_a_excel(linea.demora_eur),
        "Portes Eur": decimal_a_excel(linea.portes_eur),
        "Gasto Puerto eur": decimal_a_excel(linea.gasto_puerto_eur),
        "Aduanas Eur": decimal_a_excel(linea.aduanas_eur),
        "Otros3": decimal_a_excel(linea.otros3),
...
def _detectar_ultima_fila_completa(
    sheet_xml: str,
    *,
    min_celdas: int = _MIN_CELDAS_FILA_DATOS,
) -> int:
...
def _resolver_fila_fin_datos(
    ...
    if detectada != fila_fin_tabla:
        logger.warning(
            "generacion_fruver=%s fila_fin_tabla=%s "
...
def escribir_archivo_fruver(
    ...
    carpeta_trabajo = Path(
        tempfile.mkdtemp(prefix="fruver_write_")
    )
    salida_trabajo = carpeta_trabajo / "trabajo.xlsx"
    escritura_completada = False
    resultado: ResultadoEscrituraFruver | None = None
    ...
    try:
        ...
        with ZipFile(salida_trabajo, "r") as zipped:
            ...
            if duplicado:
                raise ContenedorDuplicadoFruverError(
        ...
        for offset in range(cantidad):
            ...
            valores = construir_valores_fila_fruver(
            ...
            if offset == 0:
                logger.info(
                    "generacion_fruver=%s hoja=%s tabla=%s "
        ...
        resultado = ResultadoEscrituraFruver(
